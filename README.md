...
When a room or doorlink component initialises, its `update` hook calls `building.buildRoom` or `building.buildDoorlink`. These methods add the element to a dirty set and call `requestBuild`, which schedules a single `requestAnimationFrame` callback — multiple calls within the same frame are coalesced by a `buildPending` flag. Because `requestAnimationFrame` is asynchronous, the callback always runs after A-Frame's synchronous component initialisation has finished, so all wall and doorhole data is fully populated by the time the build executes.
