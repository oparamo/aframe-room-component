...
| `a-opening` | `opening` | Marks a wall so a doorlink can connect to it. | (none) |
| `a-doorlink` | `doorlink` | Connects two openings and positions them along their walls. | `from`, `to`, `position`, `width`, `height`, `floor-height` |
...
An `a-opening` must be a child of an `a-wall`. It marks where a doorlink connection cuts through the wall. It can have any A-Frame entity as a child (e.g. a door model or window frame). Do **not** set a `position` on an `a-opening` — its position is controlled by the `a-doorlink` it is connected to.

An `a-doorlink` can be a child of an `a-scene` or of an `a-wall` (but **not** of an `a-opening`). Its world position is used to automatically place the two connected openings as close to it as possible on their respective walls. Whether to parent the doorlink to the scene or to one of the walls depends on your layout — it affects whether the doorway moves with a room or stays fixed. Set `floor-height` to raise the opening above the wall baseline to create a window.
...
When a room or doorlink component initialises, its `update` hook calls `building.buildRoom` or `building.buildDoorlink`. These methods add the element to a dirty set and call `requestBuild`, which schedules a single `requestAnimationFrame` callback — multiple calls within the same frame are coalesced by a `buildPending` flag. Because `requestAnimationFrame` is asynchronous, the callback always runs after A-Frame's synchronous component initialisation has finished, so all wall and opening data is fully populated by the time the build executes.
...
Rooms are always built before doorlinks because building a room populates the world-space vertices on its openings, and doorlinks consume those vertices to construct the tunnel geometry connecting them. When a room is rebuilt at runtime, its connected doorlinks are automatically added to the dirty set so their geometry stays in sync.
